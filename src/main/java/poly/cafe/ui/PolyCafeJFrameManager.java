...
public class PolyCafeJFrameManager extends javax.swing.JFrame implements PolyCafeController {
    ...
    public PolyCafeJFrameManager() {
        initComponents();
        init(); // Gọi init() để thiết lập khung
    ...
    private void displayUserInfo() {
        if (XAuth.user != null) {
            ...
            try {
                ImageIcon icon = new ImageIcon(XAuth.user.getPhoto());
    ...
    private void checkAccess() {
        if (XAuth.user == null || !XAuth.user.isManager()) {
            // Vô hiệu hóa các nút quản lý nếu không phải Manager
            ...
            XDialog.alert("Bạn không có quyền truy cập các chức năng quản lý!");
        }
    }

    @Override
    public void dispose() {
        XJdbc.closeConnection(); // Đóng kết nối database trước khi thoát
        super.dispose();
    }
